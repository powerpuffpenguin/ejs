...
class Test16 {
    ...
    test(name: string, view: SetViewN) {
        ...
        assert.equal(-1, view.getInt(0 * n), `${name} big int`)
        assert.equal(-32768, view.getInt(1 * n), `${name} big int`)
        assert.equal(32767, view.getInt(2 * n), `${name} big int`)
        assert.equal(0, view.getInt(3 * n), `${name} big int`)

        assert.equal(1, view.getUint(5 * n), `${name} big uint`)
        assert.equal(65534, view.getUint(6 * n), `${name} big uint`)
        assert.equal(65535, view.getUint(7 * n), `${name} big uint`)
        assert.equal(0, view.getUint(8 * n), `${name} big uint`)

        assert.equal(this.big, hex.encodeToString(view.buffer), `${name} big`)
