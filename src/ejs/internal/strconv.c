...
static size_t bsearch16(uint16_t *a, size_t a_len, uint16_t x)
{
    size_t i = 0, j = a_len;
    size_t h;
    while (i < j)
...
static size_t bsearch32(uint32_t *a, size_t a_len, uint32_t x)
{
    size_t i = 0, j = a_len;
    size_t h;
    while (i < j)
...
BOOL ppp_strconv_is_print(ppp_utf8_rune_t r)
{
    ...
    {
        uint16_t rr = r;
        uint16_t *isPrint = isPrint16;
        size_t isPrint_len = sizeof(isPrint16) / sizeof(uint16_t);
        uint16_t *isNotPrint = isNotPrint16;
        size_t isNotPrint_len = sizeof(isNotPrint) / sizeof(uint16_t);
    ...
    uint16_t *isNotPrint = isNotPrint32;
    size_t isNotPrint_len = sizeof(isNotPrint32) / sizeof(uint16_t);
