declare namespace deps {
    ...
    function enc_ofb(state: Pointer, blocksize: number, plaintext: string | Uint8Array): Uint8Array
    function enc_ofb(state: Pointer, blocksize: number, plaintext: string | Uint8Array, ciphertext: Uint8Array): number
    function dec_ofb(state: Pointer, blocksize: number, ciphertext: Uint8Array): Uint8Array
    function dec_ofb(state: Pointer, blocksize: number, ciphertext: Uint8Array, plaintext: Uint8Array): number
...
export class OFB {
    ...
    encrypt(plaintext: string | Uint8Array): Uint8Array {
        const state = this.state
        return deps.enc_ofb(state.p, state.blocksize, plaintext)
    }
    /**
     * Encrypt plaintext using OFB mode
     * @param ciphertext encrypted data
    ...
    encryptTo(ciphertext: Uint8Array, plaintext: string | Uint8Array): number {
        const state = this.state
        return deps.enc_ofb(state.p, state.blocksize, plaintext, ciphertext)
    }
    /**
     * Decrypt ciphertext using OFB mode
     * @param ciphertext data to be decrypted
    ...
    decrypt(ciphertext: Uint8Array): Uint8Array {
        const state = this.state
        return deps.dec_ofb(state.p, state.blocksize, ciphertext)
    }
    /**
     * Decrypt ciphertext using OFB mode
     * @param plaintext decrypted data
    ...
    decryptTo(plaintext: Uint8Array, ciphertext: Uint8Array): number {
        const state = this.state
        return deps.dec_ofb(state.p, state.blocksize, ciphertext, plaintext)
    }
}