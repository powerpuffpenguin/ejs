...
static duk_ret_t on_http_conn_close_impl(duk_context *ctx)
{
    ...
    duk_get_prop_lstring(ctx, 0, "cbc", 3);
...
static duk_ret_t create_http_link(duk_context *ctx)
{
    struct evhttp_connection *conn = duk_require_pointer(ctx, 0);
    duk_pop(ctx);
    ejs_core_t *core = ejs_require_core(ctx);

    http_link_t *p = ejs_push_finalizer_object(ctx, sizeof(http_link_t), ejs_default_finalizer);
    p->conn = conn;
    p->core = core;
    return 1;
}
static duk_ret_t close_http_link(duk_context *ctx)
{
    duk_get_prop_lstring(ctx, 0, "p", 1);
...
{
    http_link_t *p = duk_require_pointer(ctx, -1);
    duk_pop(ctx);

    if (!ejs_stash_get_pointer(ctx, p->conn, EJS_STASH_NET_HTTP_CONN))
    {
        return 0;
    }
    duk_get_prop_lstring(ctx, -1, "cb", 2);
...
} on_http_request_ecb_args_t;

static duk_ret_t on_http_request_ecb_impl(duk_context *ctx)
{
    on_http_request_ecb_args_t *p = duk_require_pointer(ctx, -1);
    duk_pop(ctx);

    if (!ejs_stash_get_pointer(ctx, p->p->conn, EJS_STASH_NET_HTTP_CONN))
    {
        return 0;
    }
    duk_get_prop_lstring(ctx, -1, "cb", 2);
    duk_push_number(ctx, p->e);
    duk_call(ctx, 1);
    return 0;
}
static void on_http_request_ecb(enum evhttp_request_error e, void *userdata)
{
    on_http_request_ecb_args_t args = {
        .e = e,
        .p = userdata,
    };
    ejs_call_callback_noresult(args.p->core->duk, on_http_request_ecb_impl, &args, 0);
}
...
static duk_ret_t create_http_request_impl(duk_context *ctx)
{
    ...
    args->req = evhttp_request_new(on_http_request_cb, args->link);
    if (!args->req)
    {
...
static duk_ret_t create_http_request(duk_context *ctx)
{
    create_http_request_args_t args = {
        .link = duk_require_pointer(ctx, 0),
        .req = 0,
    };
    duk_pop(ctx);
...
static duk_ret_t cancel_request(duk_context *ctx)
{
    evhttp_cancel_request(duk_require_pointer(ctx, 0));
    return 0;
}
...
EJS_SHARED_MODULE__DECLARE(net_http)
{
    ...
    duk_push_object(ctx);
    {
        ...
        duk_push_c_lightfunc(ctx, create_http_link, 1, 1, 0);
        duk_put_prop_lstring(ctx, -2, "create_http_link", 16);
        duk_push_c_lightfunc(ctx, close_http_link, 1, 1, 0);
        duk_put_prop_lstring(ctx, -2, "close_http_link", 15);
        ...
        duk_push_c_lightfunc(ctx, cancel_request, 1, 1, 0);
